...
uint32_t __attribute__((section (".bss"))) __attribute__ ((aligned)) screenbuffer[220*2]; // TODO: the *2 is to prevent buffer overflow
...
void drawBitmapPal16(const uint16_t screen_y, const int16_t bmp_x, const int16_t bmp_y, const uint16_t* palette, const uint8_t* bitmap) {
	if(screen_y>=bmp_y && screen_y<bmp_y+bitmap[1]) {
		...
		for(uint8_t x = 0; x < bitmap[0]/2; x++) {
			if(bmp_x+x*2 < 220) {
				...
				// ------------RRRRRRGGGGGGBBBBBB--
				// ----------------RRRRRGGGGGGBBBBB
				if(((twocolors>>4)&0xF)!=1) screenbuffer[bmp_x+x*2+0] = ((col16_1&0xF800) << 4) + ((col16_1&0x07E0) << 3) + ((col16_1&0x001F) << 2);
				if(((twocolors>>0)&0xF)!=1) screenbuffer[bmp_x+x*2+1] = ((col16_2&0xF800) << 4) + ((col16_2&0x07E0) << 3) + ((col16_2&0x001F) << 2);
			}
		}
	}
}

void drawBitmapPal4(const uint16_t screen_y, const int16_t bmp_x, const int16_t bmp_y, const uint16_t* palette, const uint8_t* bitmap) {
	if(screen_y>=bmp_y && screen_y<bmp_y+bitmap[1]) {
		uint16_t bmpy = screen_y-bmp_y;
		for(uint8_t x = 0; x < bitmap[0]/4; x++) {
			if(bmp_x+x*4 < 220) {
				...
				// ------------RRRRRRGGGGGGBBBBBB--
				// ----------------RRRRRGGGGGGBBBBB
				if(((twocolors>>6)&0xF)!=0) screenbuffer[bmp_x+x*4+0] = ((col16_1&0xF800) << 4) + ((col16_1&0x07E0) << 3) + ((col16_1&0x001F) << 2);
				if(((twocolors>>4)&0xF)!=0) screenbuffer[bmp_x+x*4+1] = ((col16_2&0xF800) << 4) + ((col16_2&0x07E0) << 3) + ((col16_2&0x001F) << 2);
				if(((twocolors>>2)&0xF)!=0) screenbuffer[bmp_x+x*4+2] = ((col16_3&0xF800) << 4) + ((col16_3&0x07E0) << 3) + ((col16_3&0x001F) << 2);
				if(((twocolors>>0)&0xF)!=0) screenbuffer[bmp_x+x*4+3] = ((col16_4&0xF800) << 4) + ((col16_4&0x07E0) << 3) + ((col16_4&0x001F) << 2);
			}
		}
	}
}
