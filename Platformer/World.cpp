...
uint8_t World::render() {
  ...
  display.setup(cnt++, bg_001_pal, bg_001_data, 0xf, PaletteSize::PAL2, xOff-bg_001_data[0], LCDHEIGHT/2 - bg_001_data[1]/2 + yOff);
	display.setup(cnt++, bg_001_pal, bg_001_data, 0xf, PaletteSize::PAL2, xOff, LCDHEIGHT/2 - bg_001_data[1]/2 + yOff);

  for (int16_t yt = YCentralTile - 6; yt < YCentralTile + 7; yt++) {
    for (int16_t xt = XCentralTile - 7; xt < XCentralTile + 8; xt++) {
      if (xt < 0) continue;
      if (yt < 0) continue;
			if (xt >= width) continue;
			if (yt >= height) continue;
